...
- API Key 只读仪表盘：按 API Key 与授权的认证文件范围查看概览、分析、请求事件和认证文件
...
启用登录保护后，管理员可以在 Dashboard 的“设置 → API Key 设置”中，为每个 CPA API Key 配置“可见认证文件”。从下拉列表多选当前有效的认证文件，无需手动填写名称。

- API Key 用户登录后只能查看“概览、分析、请求事件、认证文件”四个只读页面。
- 所有数据同时强制限定为该 API Key 和已授权认证文件对应的 OAuth 事件；浏览器参数不能扩大范围。
- 认证文件页只允许读取已缓存的额度，不允许刷新、巡检、重置额度、修改别名或读取原始请求日志。
- 保存空范围会立即禁止该 API Key 的 Viewer 访问；认证文件被 CPA 删除或重命名后也会自动拒绝访问，直到管理员更新配置。

范围配置保存在 Keeper SQLite 数据库中，按认证文件名解析为当前 `auth_index`，因此 CPA 重载认证文件导致索引变化时无需重新授权。
