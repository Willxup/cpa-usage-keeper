...
func sumRawUsageWindowTokenStats(db *gorm.DB, authIndex string, start time.Time, end *time.Time) ([]usageWindowTokenStats, error) {
	...
	query := db.Model(&entities.UsageEvent{}).
		...
		Select("model, COALESCE(SUM(total_tokens), 0) AS total_tokens, COALESCE(SUM(input_tokens), 0) AS input_tokens, COALESCE(SUM(output_tokens), 0) AS output_tokens, COALESCE(SUM(cached_tokens), 0) AS cached_tokens, COALESCE(COUNT(*), 0) AS request_count").
...
func sumHourlyUsageWindowTokenStats(db *gorm.DB, authIndex string, start time.Time, end time.Time) ([]usageWindowTokenStats, error) {
	...
	query := db.Model(&entities.UsageOverviewHourlyStat{}).
		...
		Select("model, COALESCE(SUM(total_tokens), 0) AS total_tokens, COALESCE(SUM(input_tokens), 0) AS input_tokens, COALESCE(SUM(output_tokens), 0) AS output_tokens, COALESCE(SUM(cached_tokens), 0) AS cached_tokens, COALESCE(SUM(request_count), 0) AS request_count").
...
func usageWindowStatsFromTokenStats(rows []usageWindowTokenStats, pricingByModel map[string]entities.ModelPriceSetting) UsageWindowStats {
	...
	for _, row := range rows {
		...
		stats.Cost += helper.CalculateUsageTokenCost(helper.UsageTokenCostInput{InputTokens: row.InputTokens, OutputTokens: row.OutputTokens, CachedTokens: row.CachedTokens, Requests: row.RequestCount}, pricing)
	}
	// 返回最终窗口统计。
	return stats
}
