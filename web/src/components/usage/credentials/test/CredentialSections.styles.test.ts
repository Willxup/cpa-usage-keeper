...
describe('Credential section styles', () => {
  it('keeps Auth Files and AI Provider row sizing separate', () => {
    ...
    expect(aiProviderSectionSource).toContain('rowClassName={relayRowClassName}')
    expect(aiProviderSectionSource).toContain('aiProviderCredentialRowRelay')
  ...
  it('lets Auth Files quota bars wrap before their blocks overlap', () => {
    ...
    expect(credentialStyles).toMatch(/\.credentialHealthPanel\s*\{[\s\S]*?grid-template-columns:\s*minmax\(211px, max-content\) minmax\(120px, 1fr\);/)
  ...
  it('keeps Auth Files inspection separate from the quota refresh pill', () => {
    ...
    expect(credentialStyles).toMatch(/\.credentialInspectionProgressFill\s*\{[^}]*background:\s*#22c55e;/)
